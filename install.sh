...
echo "== SageAttention (the reference attention backend) =="
pip install sageattention==2.2.0 || {
  echo "WARNING: sageattention install failed. Either install it manually"
  echo "         (needs nvcc matching your torch) or set"
  echo "         use_sage_attention: false in the config."
}
